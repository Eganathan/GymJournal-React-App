...
function ExerciseGroup({ exercise, sessionId, isCompleted: sessionCompleted }) {
  ...
  // Memoize best 1RM calculation to avoid reducing the PBs array on every timer tick re-render
...
export default function WorkoutActive() {
  ...
  // Memoize categories and equipment into Maps for O(1) lookups during the exercise list render loop
  // This avoids O(N*M) performance degradation when searching large arrays inside .map()
  const categoriesMap = useMemo(() => {
    const map = new Map();
    exCategories?.forEach((cat) => {
  ...
  const equipmentMap = useMemo(() => {
    const map = new Map();
    exEquipment?.forEach((eq) => {
      map.set(String(eq.id), eq);
    });
    return map;
  }, [exEquipment]);
