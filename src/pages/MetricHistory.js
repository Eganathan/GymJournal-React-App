...
export default function MetricHistory() {
  ...
  const history = useMetricsStore((s) => s.getHistory(metricType));
  ...
  const customDef = customMetricDefs.find((d) => d.key === metricType);
              ...
              return (
                <div key={entry.id} className="card flex items-center justify-between animate-fade-in">
                  <div>
                    ...
                    <p className="text-xs text-neutral-600 mt-0.5">{entry.date}</p>
                  </div>
                  <button
                    onClick={() => { if (window.confirm('Delete this entry?')) deleteEntry(entry.id); }}
